...
internal class Program
{
    public static async Task Main(string[] args)
    {
        ...
        srucTable.AddColumn(" Boosted ",    coop => StringFormatter.Centered($"{coop.BoostedCount}", 9));
        srucTable.AddColumn(" Tokens ",     coop => StringFormatter.Centered($"{coop.TotalTokens}", 8));
        srucTable.AddColumn(" Duration ",   coop => StringFormatter.Centered(coop.PredictedDuration.DurationInSeconds < 8640000 ? coop.PredictedDuration.Format() : "too long", 10));
        ...
        Console.WriteLine($"""
            Last updated: {(new DiscordTimestamp(secondsSinceEpoch)).Format(DiscordTimestampDisplay.Relative)}
